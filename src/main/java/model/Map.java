...
public class Map {
     ...
     * once there are more than rows or columns, as it expects each row
    ...
    private void populateMap(int rows, int columns) {
        ...
        for (int r = 0; r < rows; r++) {
            ...
            for (int c = 0; c < columns; c++) {
                ...
                if (value < div(total, 4.0)) {
                    type = TileType.WATER;
                }
                if (value > div(total, 2.5) && value < div(total, 1.4)
                        && c < div(total, 4.0) && r > div(total, 4)) {
                    type = TileType.DESERT;
                } else if (value > div(total, 2.0)
                        && value < div(total, 1.25)) {
                    type = TileType.FOREST;
                }
                if (value > div(total, 1.4) && value < div(total, 1.11)) {
                    type = TileType.MOUNTAIN;
                } else if (value >= div(total, 1.11)) {
                    type = TileType.ICE;
                }
    ...
    private int div(int total, double num) {
    ...
    public static void addEnemies(Bandit bandit, int numEnemies) {
        for (int i = 0; i < numEnemies; i++) {
            int r = new Random().nextInt(mapRow);
            int c = new Random().nextInt(mapCol);
            putSettlement("Bandit Hideout", bandit, mapCol - 1, 0);
            while (!map[r][c].isEmpty()) {
                //try to get new coords
                r = new Random().nextInt(mapRow);
                c = new Random().nextInt(mapCol);
            }
            ...
            if (!current.isEmpty()
                ...
                for (int dr = current.getRow() - 1; dr <= current.getRow() + 1;
                    ...
                    for (int dc = current.getCol() - 1;
                        ...
                        // experimental
                        if  (dr >= 0 && dr < mapRow && dc >= 0 && dc < mapCol) {
