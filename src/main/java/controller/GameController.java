...
public class GameController {
    ...
    public static void setLastClicked(TerrainTileFX last) {
        ...
        if (state == GameState.ATTACKING) {
            ...
            if (move(last)) {
                File sound = new File("src/main/java/view/"
                        + "sound_effects/movebutton.wav");
                ...
                alert.setTitle("Can not Move");
                alert.setHeaderText("Can not move");
                alert.setContentText("You can not move!!!\n"
                        + "Try to end turn to able to move."
                        + " You can only move one spot at a time.");
                alert.showAndWait();
            }
        }
        //update lastClicked
    ...
    private static void attackTile(TerrainTile attacker, TerrainTile enemy) {
        if (enemy.isEmpty()
            ...
            state = GameState.NEUTRAL;
            Alert alert = new Alert(AlertType.INFORMATION);
            alert.setTitle("Can not Attack");
            alert.setHeaderText("Can not Attack");
            alert.setContentText("You can not Attact!!!\n"
                    + "Try to end turn to able to attack.\n"
                    + "Or you should move closer to enemy"
                    + " to able to attack");
            alert.showAndWait();
            return;
        }
        if (attacker.getOccupant().getOwner() == playerCivilization) {
            File sound = new File("src/main/java/view/"
                + "sound_effects/attackbutton.wav");
            Media media = new Media(sound.toURI().toString());
            MediaPlayer soundPlayer = new MediaPlayer(media);
            soundPlayer.play();
            GameController.updateResourcesBar();
    ...
    public static void ai() {
        ...
        for (TerrainTile tile : bandits) {
            ...
            if (newRow >= 0 && newRow < mapRow
                    && newCol >= 0 && newCol < mapCol) {
    ...
    private static TerrainTile getRandomAdjacentTile(int row, int col) {
        ...
        while (!(r >= 0 && r < mapRow && c > 0 && c < mapCol)) {
